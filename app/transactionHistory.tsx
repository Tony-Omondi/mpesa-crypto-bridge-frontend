...
export default function TransactionHistory() {
    ...
    try {
      ...
      if (walletAddress) {
        try {
          ...
          const [ethRes, tokenRes] = await Promise.all([apiClient.get(ethUrl), apiClient.get(tokenUrl)]);

          let externalTxs: any[] = [];

          if (ethRes.data?.status === "1") {
            externalTxs = [...externalTxs, ...ethRes.data.result
              .filter((tx: any) => tx.value !== "0")
              .map((tx: any) => ({
                id: tx.hash, type: "TRANSFER",
                ...
                tx_hash: tx.hash, tokenSymbol: 'ETH'
              }))];
          }

          if (tokenRes.data?.status === "1") {
            externalTxs = [...externalTxs, ...tokenRes.data.result.map((tx: any) => ({
              id: tx.hash, type: "TRANSFER",
              from_address: tx.from, to_address: tx.to,
              amount: (Number(tx.value) / Math.pow(10, Number(tx.tokenDecimal))).toFixed(2),
              status: "COMPLETED",
              created_at: new Date(Number(tx.timeStamp) * 1000).toISOString(),
              tx_hash: tx.hash, tokenSymbol: tx.tokenSymbol || 'NIT'
            }))];
          }
        ...
        } catch (e) { console.warn("⚠️ Arbiscan Failed:", e); }
      }
  ...
  useFocusEffect(useCallback(() => {
    if (walletAddress) { setLoading(true); fetchHistory(); }
  }, [walletAddress]));
  ...
  const getTxUI = (item: any) => {
    ...
    if (isDeposit) return { category: 'Deposits', title: "M-Pesa Deposit", icon: "arrow-down-outline", color: COLORS.success, prefix: "+", symbol: 'NIT' };
    if (isSent) return { category: 'Sent', title: `Sent ${symbol}`, icon: "arrow-up-outline", color: COLORS.error, prefix: "-", symbol };
  ...
  }, [history, activeFilter, walletAddress]);

  // ✅ Tap → Transaction Receipt
  const handleTxPress = (item: any) => {
    ...
    router.push({
      ...
      params: {
        ...
        status: item.status, tx_hash: item.tx_hash || '',
        from_address: item.from_address || '',
        to_address: item.to_address || '',
        created_at: item.created_at,
        tokenSymbol: item.tokenSymbol || 'NIT',
      }
    });
  };
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity style={styles.historyItem} onPress={() => handleTxPress(item)} activeOpacity={0.7}>
        ...
        <View style={styles.amountContainer}>
          ...
          <Text style={[styles.statusText, { color: isPending ? COLORS.warning : COLORS.textSecondary }]}>
            {item.status}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  iconButton: { width: 40, height: 40, justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.surface, borderRadius: 20, borderWidth: 1, borderColor: COLORS.border },
  ...
  historyIcon: { width: 48, height: 48, borderRadius: 24, justifyContent: 'center', alignItems: 'center', marginRight: 16 },
  ...
  emptyIconBg: { width: 80, height: 80, borderRadius: 40, backgroundColor: COLORS.surface, justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  emptyText: { color: COLORS.textPrimary, fontSize: 18, fontWeight: '600' },
  emptySubText: { color: COLORS.textSecondary, fontSize: 14, textAlign: 'center', paddingHorizontal: 40, lineHeight: 20 },
});
